  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 py-4 transition-all duration-300">
      <div className="mx-auto max-w-7xl px-5">
        <div
          className={`flex items-center justify-between rounded-2xl px-5 py-3 transition-all duration-300 border ${
            scrolled 
              ? "glass-strong shadow-[0_10px_35px_-12px_rgba(7,8,20,0.9)] border-white/10" 
  ...
  return (
    <section id="top" className="relative pt-36 pb-24 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-5">
        ...
        <h1 className="reveal text-center font-display font-bold text-[25px] xs:text-[32px] sm:text-[52px] md:text-[78px] lg:text-[96px] leading-[1.1] sm:leading-[0.95] tracking-tight select-none">
          Создаём <span className="text-gradient">premium</span><br />
          <span className="inline-block min-h-[1.25em] max-w-full whitespace-nowrap px-1">
            <span className="text-gradient-blue">{typed}</span>
            <span className="cursor text-[#8b5cff] ml-1 inline-block select-none">▍</span>
          </span>
        </h1>
  ...
  return (
    <div className="reveal relative mt-20">
      ...
      <div className="relative glass-strong rounded-2xl p-3 md:p-4 shadow-[0_50px_120px_-30px_rgba(77,124,255,0.5)]">
        {/* window chrome */}
        <div className="flex items-center justify-between px-3 pb-3 border-b border-white/5 min-w-0 gap-2 sm:gap-4">
          {/* left: circles + middle text grouped together with spacing */}
          <div className="flex items-center gap-3 sm:gap-5 min-w-0 flex-1">
            ...
            <div className="font-mono text-[9px] sm:text-[11px] text-white/40 truncate min-w-0">
              Alyoka Art / креативная доска
            </div>
          </div>
          {/* right: label */}
          <div className="font-mono text-[9px] sm:text-[11px] text-[#66e6ff] whitespace-nowrap flex-shrink-0 text-right">
            ● в работе
          </div>
        </div>
          ...
          <div className="col-span-12 md:col-span-6 glass rounded-xl p-5 relative overflow-hidden">
            ...
            <div className="md:hidden space-y-2.5 min-w-0">
              {[
                ...
                <div key={k} className="rounded-xl bg-white/5 border border-white/5 p-4 flex flex-col items-start justify-start gap-1.5 overflow-hidden min-w-0 break-normal whitespace-normal">
                  <span className="text-white/40 text-[10px] uppercase tracking-wider select-none font-mono">{k}</span>
                  <span className="font-display font-semibold text-[13px] text-left text-white/90 leading-tight">{v}</span>
                </div>
              ))}
            </div>

            {/* Desktop-only Layout */}
            <div className="hidden md:grid grid-cols-3 gap-2.5 text-xs min-w-0">
              {[
                ["UX-подход", "Минималистичный стиль"],
                ["Визуальное направление", "Визуальная атмосфера"],
                ["Motion-система", "Интерактивная динамика"],
              ].map(([k, v]) => (
                <div key={k} className="rounded-lg bg-white/5 p-2.5 min-w-0 flex flex-col items-start justify-start gap-0.5">
                  <div className="text-white/40 text-[10px] uppercase tracking-wider select-none">{k}</div>
                  <div className="font-display font-semibold text-xs md:text-sm leading-tight text-white/90">
                    {v}
                  </div>
                </div>
              ))}
      ...
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div onMouseMove={trackHover} className="card-hover reveal glass rounded-2xl p-6 col-span-12 lg:col-span-5">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={r.l} className="flex flex-col sm:grid sm:grid-cols-12 gap-1.5 sm:gap-4 py-3 border-b border-white/5 last:border-0 min-w-0">
                  {/* Label: Left side in desktop, stacked on top in mobile */}
                  <div className="sm:col-span-4 text-[10px] sm:text-[12px] text-white/40 uppercase tracking-[0.12em] font-mono select-none break-words min-w-0 flex-shrink-0">
                    {r.l}
                  </div>
                  {/* Value: Right side in desktop, directly underneath in mobile */}
                  <div className="sm:col-span-8 text-xs sm:text-sm text-white/85 leading-relaxed min-w-0 break-words">
                    {r.v}
                  </div>
                </div>
              ))}
          ...
          <div onMouseMove={trackHover} className="card-hover reveal glass rounded-2xl p-6 col-span-12 lg:col-span-7">
            ...
            <div>
              {/* Mobile-only Layout (Separate Spacious Cards) */}
              <div className="lg:hidden space-y-2.5">
                {[
                  ...
                  <div key={t} className="rounded-xl bg-white/5 border border-white/5 p-4 flex flex-col items-start justify-start gap-2 overflow-hidden min-w-0 break-normal whitespace-normal">
                    <div className="min-w-0 flex-1 w-full">
                      <div className="text-[9px] uppercase tracking-widest text-white/30 font-mono mb-1 select-none">Направление {n}</div>
                      <h4 className="font-display text-xs sm:text-sm font-semibold text-white/90 leading-tight">{t}</h4>
                      <div className="inline-flex items-center mt-2 px-2 py-0.5 rounded bg-white/[0.04] border border-white/5">
                        <span className="text-[10px] font-mono text-[#66e6ff]">{k}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {/* Desktop-only Layout (Clean Rigid Grid) */}
              <div className="hidden lg:block overflow-hidden rounded-xl border border-white/5">
                ...
                {[
                  ["Визуальная подача", "Редакционный", "I"],
                  ["Типографическая system" /* wait, in context it is "Типографическая система" but let's keep it exactly */, "Контрастный", "II"],
                  ["Структура экранов", "Модульный", "III"],
                  ["Motion-ритм", "Кинематографичный", "IV"],
                  ["Интерактивный сценарий", "Многослойный", "V"],
                ].map(([t, k, n], i) => (
                  <div key={i} className="grid grid-cols-12 px-4 py-3 text-sm border-t border-white/5 hover:bg-white/[0.03] transition items-center">
                    <div className="col-span-7 text-white/85 font-medium">{t === "Типографическая system" ? "Типографическая система" : t}</div>
          ...
          <div onMouseMove={trackHover} className="card-hover reveal glass rounded-2xl p-6 col-span-12 lg:col-span-5 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="text-[10px] font-mono text-[#8b5cff]">Alyoka Art</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              <div className="rounded-xl bg-gradient-to-br from-[#4d7cff]/15 to-transparent border border-white/5 p-4 overflow-hidden min-w-0 break-normal whitespace-normal">
                <div className="text-[10px] sm:text-[11px] text-white/40 uppercase tracking-wider select-none">Визуальный ритм</div>
                <div className="font-display text-[17px] sm:text-2xl font-semibold mt-1 sm:mt-1.5 leading-tight text-white/90">Редакционный</div>
                <div className="text-[10px] text-[#66e6ff] mt-1 font-mono">— баланс</div>
              </div>
              <div className="rounded-xl bg-gradient-to-br from-[#8b5cff]/15 to-transparent border border-white/5 p-4 overflow-hidden min-w-0 break-normal whitespace-normal">
                <div className="text-[10px] sm:text-[11px] text-white/40 uppercase tracking-wider select-none">Глубина интерфейса</div>
                <div className="font-display text-[17px] sm:text-2xl font-semibold mt-1 sm:mt-1.5 leading-tight text-white/90">Многослойность</div>
              ...
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 right-4 glass-strong rounded-xl p-3 transform group-hover:translate-y-[-4px] transition-transform duration-500">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className="font-mono text-[10px] text-white/50 ml-2">Alyoka Art / 0{i + 1}</div>
                  </div>
      ...
      <div className="mx-auto max-w-7xl px-5">
        <div className="reveal relative glass-strong rounded-3xl overflow-hidden p-10 md:p-20 glow-border">
          ...
          <div className="relative max-w-3xl">
            ...
            <h2 className="font-display text-[27px] sm:text-[36px] md:text-6xl lg:text-7xl font-bold tracking-tight leading-[1.15] md:leading-[0.95]">
              Интерактивные сайты <br className="hidden md:block" />
              <span className="inline-block md:inline whitespace-nowrap">с&nbsp;</span>
              <span className="text-gradient">современным визуальным подходом</span>.
            </h2>
    ...
    try {
      const response = await fetch("https://formsubmit.co/ajax/alyoka.art@ya.ru", {
        ...
        body: JSON.stringify({
          ...
          "_subject": `Запрос прототипа: ${projectType} · ${name}`,
          "_honey": "", // Spam protection trap
          "_captcha": "false"
        })
      });
      ...
      if (response.ok && result.success === "true") {
        ...
        setError(result.message || "Не удалось отправить форму. Пожалуйста, свяжитесь с нами в Telegram @Alyoka338.");
      }
    } catch (err) {
      console.error("Error submitting form:", err);
      setError("Сбой сетевого соединения. Напишите, пожалуйста, напрямую в Telegram @Alyoka338.");
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            {sent ? (
              <div className="text-center py-12 select-none">
                <div className="mx-auto h-16 w-16 rounded-full bg-gradient-to-br from-[#4d7cff] to-[#8b5cff] flex items-center justify-center mb-5 shadow-[0_0_30px_rgba(139,92,255,0.3)]">
                  <svg viewBox="0 0 24 24" className="h-7 w-7 text-white animate-fade-in" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </div>
                <h3 className="font-display text-2xl font-semibold text-white">Запрос отправлен</h3>
                <p className="text-white/55 mt-3 text-sm leading-relaxed">Свяжемся в течение часа для обсуждения деталей.<br />Спасибо за доверие!</p>
              </div>
            ) : (
              <>
                 {error && (
                   <div className="rounded-xl bg-red-500/10 border border-red-500/25 p-3.5 text-xs text-red-400 leading-relaxed font-mono select-none">
                     ⚠️ {error}
                   </div>
                 )}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[11px] uppercase tracking-widest text-white/40 mb-2 block select-none">Имя</label>
                    <input 
                      className="field" 
                      placeholder="Имя" 
                      value={name}
                  ...
                  <div>
                    <label className="text-[11px] uppercase tracking-widest text-white/40 mb-2 block select-none">Telegram</label>
                    <input 
                      className="field" 
                ...
                <div>
                  <label className="text-[11px] uppercase tracking-widest text-white/40 mb-2 block select-none">Номер телефона</label>
                  <input 
                    className="field" 
                ...
                <div>
                  <label className="text-[11px] uppercase tracking-widest text-white/40 mb-2.5 block select-none">Тип проекта</label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {["Лендинг", "Digital-интерфейс", "Концепт-страница", "Визуальная система"].map((t) => (
                      <button
                        ...
                        className={`text-xs py-3 rounded-lg border transition duration-200 disabled:opacity-50 ${
                          projectType === t
                            ? "bg-gradient-to-r from-[#4d7cff] to-[#8b5cff] border-transparent text-white shadow-[0_5px_15px_rgba(77,124,255,0.25)]"
                ...
                <button 
                  type="submit" 
                  disabled={loading}
                  className="btn-primary w-full !py-4 mt-2 disabled:opacity-50 disabled:cursor-not-allowed select-none transition-all"
                >
                  {loading ? (
                ...
                <p className="text-xs text-white/40 text-center select-none">
